...
public static class ToInt64Extensions
{
    public static long ToInt64(this object value, IFormatProvider provider = null) =>
        value.ConvertTo(v => Convert.ToInt64(value.ToFloat(provider), provider ?? MaestriaFluentCastSettings.Properties.NumberCulture));

    public static long? ToInt64Safe(this object value, IFormatProvider provider = null) =>
        value.ConvertToSafe(v => Convert.ToInt64(value.ToFloat(provider), provider ?? MaestriaFluentCastSettings.Properties.NumberCulture));
